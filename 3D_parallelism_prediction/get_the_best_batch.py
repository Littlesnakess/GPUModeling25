...
def get_the_best_batch(target_path, unm_threads):
    save_dict = {}
    path_list = get_stages_profiler_path(target_path)
    save_folder = os.path.dirname(target_path)
    for i in range(len(path_list)):
        print(f'loading: {path_list[i]}')
        temp_decoder = TimelineDecoder(path_list[i], num_threads=unm_threads)

        print('finding the best batch')
        train_batches = temp_decoder.get_items_by_name(['deepspeed/runtime/pipe/engine.py(312): train_batch'])
        best_train_batch = sorted(train_batches, key=lambda x: x.get('dur'))[0]
        
        save_dict['best_batch'] = best_train_batch
        
        train_batches = sorted(train_batches, key=lambda x: x.get('ts'))
        save_dict['batch_dur_list'] = [x.get('dur') for x in train_batches]

        print('finding the sub items')
        sub_items = temp_decoder.get_sub_data_by_item(best_train_batch)
        sub_items.append(best_train_batch)
        save_dict['traceEvents'] = sub_items

        result_save_folder = save_folder + '/best_batch/' + str(i)
        os.makedirs(result_save_folder, exist_ok=True)

        save_name = str(i) + '.json' 
        save_path = result_save_folder + '/' + save_name
...
def get_the_worst_batch(target_path, unm_threads):
    save_dict = {}
    path_list = get_stages_profiler_path(target_path)
    save_folder = os.path.dirname(target_path)
    for i in range(len(path_list)):
        ...
        print('finding the worst batch')
        train_batches = temp_decoder.get_items_by_name(['deepspeed/runtime/pipe/engine.py(312): train_batch'])
        worst_train_batch = sorted(train_batches, key=lambda x: x.get('dur'))[-1]
        
        save_dict['worst_batch'] = worst_train_batch
        
        train_batches = sorted(train_batches, key=lambda x: x.get('ts'))
        save_dict['batch_dur_list'] = [x.get('dur') for x in train_batches]

        print('finding the sub items')
        sub_items = temp_decoder.get_sub_data_by_item(worst_train_batch)
        sub_items.append(worst_train_batch)
        save_dict['traceEvents'] = sub_items

        result_save_folder = save_folder + '/worst_batch/' + str(i)
        os.makedirs(result_save_folder, exist_ok=True)
...
if __name__ == "__main__":
    ...
    get_the_best_batch(args.target_path, args.threads)
    # python get_the_best_batch.py --threads 16 --target_path ./vista/GPT_350M_8_1/91457/PP2_MP2_DP2_ZERO1
    ...
    # python vista_parallel_timeline_decoder_new.py --threads 16 --target_path ./vista/GPT_350M_8_1/91457/PP2_MP2_DP2_ZERO1
    ...
    get_the_worst_batch(args.target_path, args.threads)
